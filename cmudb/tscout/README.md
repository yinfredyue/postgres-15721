...
1. `model.py` converts C types to BPF types, defines Operating Units (OUs) and metrics to be collected.
2. `tscout.py` uses the following input:
    - OUs and metrics from model.py
    - collector.c, markers.c, probes.c as codegen templates
3. `tscout.py` then attaches to the postmaster.

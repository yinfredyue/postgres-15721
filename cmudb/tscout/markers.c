...
struct SUBST_OU_features {
  s64 query_id;
  s32 plan_node_id;
  s32 left_child_plan_node_id;
  s32 right_child_plan_node_id;
  u32 db_id;
  u64 payload;
  u64 statement_timestamp;
};
...
void SUBST_OU_features(struct pt_regs *ctx) {
  ...
  s32 ou_instance = 0;
  bpf_usdt_readarg(1, ctx, &ou_instance);
  bpf_usdt_readarg(1, ctx, &features->plan_node_id);
  bpf_usdt_readarg(2, ctx, &features->query_id);
  bpf_usdt_readarg(3, ctx, &features->db_id);
  bpf_usdt_readarg(4, ctx, &features->statement_timestamp);
  bpf_usdt_readarg(5, ctx, &features->left_child_plan_node_id);
  bpf_usdt_readarg(6, ctx, &features->right_child_plan_node_id);
  SUBST_OU_complete_features.update(&ou_instance, features);
}

void SUBST_OU_features_payload(struct pt_regs *ctx) {
  s32 ou_instance = 0;
  bpf_usdt_readarg(1, ctx, &ou_instance);
  ...
  bpf_usdt_readarg(2, ctx, &features->payload);
  SUBST_OU_complete_features.update(&ou_instance, features);
}
...
void SUBST_OU_flush(struct pt_regs *ctx) {
  ...
  __builtin_memcpy(&(output->query_id), features, sizeof(struct SUBST_OU_features));

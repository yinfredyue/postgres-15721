...
static bool test_per_tuple = false;
...
main(int argc, char *argv[])
{
	...
	printf("Press ENTER to start profile:\n");
	{
		int ch;
		while ((ch = getchar()) != '\n' && ch != EOF) {
		}
...
handle_args(int argc, char *argv[])
{
	...
	if (argc > 1)
	{
		...
		{
			printf(_("Usage: %s [-d DURATION] [-p]\n"), progname);
			exit(0);
		}
		if (strcmp(argv[1], "--version") == 0 || strcmp(argv[1], "-V") == 0)
		{
			puts("tscout_test_timing (PostgreSQL) " PG_VERSION);
			exit(0);
		}
	}
...
test_timing(unsigned int duration)
{
	...
	if (test_per_tuple) {
		TS_MARKER(ExecAgg_features, -1, 0, 0LL, 0, 0LL, -1, -1);
	}
	...
	while (time_elapsed < total_time)
	{
		...
		TS_MARKER(ExecAgg_begin, -1);
		TS_MARKER(ExecAgg_end, -1);

		if (!test_per_tuple) {
			TS_MARKER(ExecAgg_flush, -1);
		}
